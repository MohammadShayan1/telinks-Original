...
// Input validation
$itm_name = filter_input(INPUT_POST, 'nname', FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$itm_email = filter_input(INPUT_POST, 'nemail', FILTER_SANITIZE_EMAIL);

// Check if inputs are not empty
if (!$itm_name || !$itm_email) {
    $_SESSION['error_message'] = 'Please provide both your name and email.';
    header("Location: ../../home?#newsletter");
    exit();
}

// Validate email format
if (!filter_var($itm_email, FILTER_VALIDATE_EMAIL)) {
    $_SESSION['error_message'] = 'Please provide a valid email address.';
    header("Location: ../../home?#newsletter");
    exit();
}

// Include necessary configuration and ensure the global $sql is initialized
global $sql;

// Ensure that the $sql object is initialized
if (!$sql) {
    error_log('SQL object not initialized');
    $_SESSION['error_message'] = 'Database connection not initialized. Please try again later.';
    header("Location: ../../home?#newsletter");
    exit();
}
...
if ($stmt->num_rows > 0) {
    ...
    // Insert new subscriber into the newsletter table using prepared statements
    $insert_query = "INSERT INTO newsletter (n_name, n_email) VALUES (?, ?)";
    $stmt = $sql->prepare($insert_query);

    if (!$stmt) {
        error_log('Error preparing insert statement: ' . $sql->getError());
        $_SESSION['error_message'] = 'There was an error processing your request. Please try again later.';
        header("Location: ../../home?#newsletter");
        exit();
    }
